...
class ChesapeakeCVPR(GeoDataset):
    ...
    * https://doi.org/10.1109/cvpr.2019.01301
    ...
    def init_parent(self, transforms):
        super().__init__(transforms)
...
class ChesapeakeICLR(ChesapeakeCVPR):
    valid_layers = [
        "naip-new", # real images
        "syn_lambda_0", # synthetic images
        "syn_lambda_2",
        "syn_lambda_4",
        "syn_lambda_6",
        "syn_lambda_8",
        "syn_lambda_10",
        "lc", # label for landcover (6 classes)
        "buildings", # label for buildings
    ]
    ...
    files = [
        ...
        "md_1m_2013_extended-debuffered-train_tiles",
        "md_1m_2013_extended-debuffered-val_tiles",
        "spatial_index.geojson", # TODO: can be named to the new spatial_index file
    ]
    files_dict = {
        "md-test":"md_1m_2013_extended-debuffered-test_tiles",
        "md-train":"md_1m_2013_extended-debuffered-train_tiles",
        "md-val":"md_1m_2013_extended-debuffered-val_tiles",
    }

    def __init__(
            self,
            root: str = "data",
            splits: Sequence[str] = ["md-train"],
            layers: Sequence[str] = ["naip-new", "lc"],
            transforms: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None,
            cache: bool = True,
            download: bool = False,
        ...
        self.files = [self.files_dict[split] for split in splits] + ["spatial_index.geojson"] # TODO: can be named to the new spatial_index file
        ...
        if self.mix_rate == 2.0 or self.mix_rate == 3.0:
            for i, split in enumerate(splits):
                if split == "md-train":
                    splits[i] = "md-train-200" if self.mix_rate == 2.0 else "md-train-300"
                if split == "md-val" or split == "md-test":
                    assert self.mix_rate == 0.0
                    
        self._verify()

        super().init_parent(transforms)
        ...
        mint: float = 0
        maxt: float = sys.maxsize
        with fiona.open(os.path.join(root, "spatial_index.geojson"), "r") as f: # TODO: can be named to the new spatial_index file
            for i, row in enumerate(f):
                if row["properties"]["split"] in splits:
                    box = shapely.geometry.shape(row["geometry"])
                    minx, miny, maxx, maxy = box.bounds
                    coords = (minx, maxx, miny, maxy, mint, maxt)
                    if row["properties"]["split"] in ["md-val", "md-test"]:
                        self.index.insert(
                            i,
                            coords,
                            {
                        ...
                        if self.mix_rate <= 1.0:
                            self.index.insert(
                                i,
                                coords,
                                {
                                    "naip-new": row["properties"]["naip-new"],
                                    "syn_lambda_0": row["properties"]["syn_lambda_0"], 
                                    "syn_lambda_2": row["properties"]["syn_lambda_2"], 
                                    "syn_lambda_4": row["properties"]["syn_lambda_4"], 
                                    "syn_lambda_6": row["properties"]["syn_lambda_6"], 
                                    "syn_lambda_8": row["properties"]["syn_lambda_8"], 
                                    "syn_lambda_10": row["properties"]["syn_lambda_10"], 
                                    "lc": row["properties"]["lc"],
                                    "buildings": row["properties"]["buildings"]
                                },
                            )
                        else: # mix_rate = 2.0 or 3.0
                            assert row["properties"]["split"] in ["md-train-200", "md-train-300"]
                            self.index.insert(
                                i,
                                coords,
                                {
    ...
    def _getfn(self, layer, filenames):
        diversity2layer = {
            0: "syn_lambda_0",
            2: "syn_lambda_2",
            4: "syn_lambda_4",
        ...
        if layer == "naip-new":
            if self.mix_rate > 0.0 and self.mix_rate <= 1.0:
                pivot = random.random()
                if pivot < self.mix_rate:
                    layer = diversity2layer[self.diversity]
            elif self.mix_rate == 2.0 or self.mix_rate == 3.0:
                assert self.diversity == 6
                layer = diversity2layer[self.diversity]
            else:
                pass
    ...
    def __getitem__(self, query: BoundingBox) -> Dict[str, Any]:
        ...
        if (len(filepaths) == 3 and self.mix_rate == 3.0) or (len(filepaths) == 2 and self.mix_rate == 2.0):
        ...
        if len(filepaths) == 0:
            ...
            filenames = filepaths[0] # a sample row in spatial_index.geojson
            query_geom_transformed = None  # is set by the first layer
            ...
            layer = "naip-new"
            dst_crs = None
            with rasterio.open(os.path.join(self.root, filenames[layer])) as original_f:
                dst_crs = original_f.crs
                query_box_transformed = shapely.ops.transform(
                    self.p_transformers[dst_crs.to_string().lower()], query_box
                ).envelope
                query_geom_transformed = shapely.geometry.mapping(
                    query_box_transformed
                )
            ...
            for layer in self.layers:
                fn = self._getfn(layer, filenames)

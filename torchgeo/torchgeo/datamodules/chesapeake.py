...
from ..datasets import ChesapeakeCVPR, ChesapeakeICLR, stack_samples
from ..samplers.batch import RandomBatchGeoSampler
from ..samplers.single import GridGeoSampler


class ChesapeakeDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        for state in train_splits + val_splits + test_splits:
            assert state in dataset_class.splits
        ...
        if dataset_class.__name__ == 'ChesapeakeCVPR' and self.use_prior_labels:
            self.layers = [
                "naip-new",
    ...
    def prepare_data(self) -> None:
        ...
        if self.kwargs.get("download", False):
            self.dataset_class(splits=self.train_splits, layers=self.layers, **self.kwargs)
...
class ChesapeakeCVPRDataModule(ChesapeakeDataModule):
    def __init__(
        self,
        train_splits: List[str],
        val_splits: List[str],
        ...
        **kwargs: Any,
        ...
        super().__init__(train_splits, val_splits, test_splits, patches_per_tile, patch_size, batch_size, num_workers, class_set, use_prior_labels, prior_smoothing_constant, ChesapeakeCVPR, **kwargs)


class ChesapeakeICLRDataModule(ChesapeakeDataModule):
    def __init__(
        self,
        train_splits: List[str],
        val_splits: List[str],
        ...
        use_prior_labels: bool = False,
        prior_smoothing_constant: float = 1e-4,
        dataset_class = ChesapeakeICLR,
        mix_rate = 0.0,
        diversity = 6,
        channels: int = 3,
        **kwargs: Any,
    ) -> None:
        super().__init__(train_splits, val_splits, test_splits, patches_per_tile, patch_size, batch_size, num_workers, class_set, use_prior_labels, prior_smoothing_constant, dataset_class, **kwargs)

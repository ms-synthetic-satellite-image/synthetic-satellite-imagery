...
def multiclass_dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    ...
    for channel in range(input.shape[1]): # number of classes
        score = dice_coeff(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon)
        dice += score
    return dice / input.shape[1]

def labelwise_dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    # Average of Dice coefficient for all classes
    assert input.size() == target.size()
    dices = []
    for channel in range(input.shape[1]): # number of classes
